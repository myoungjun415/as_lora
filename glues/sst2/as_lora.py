...
NUM_CLIENTS = 6
ACTIVE_CLIENTS = 6
DATA_DIR = Path("sst2_federated_6clients_alpha0.5")
...
def select_param_groups_by_layer_modes(
    ...
    return params_A, params_B, params_score_only
...
def make_client_loaders(batch_size: int, max_len: int = MAX_LEN) -> List[DataLoader]:
    ...
    for cid in range(NUM_CLIENTS):
        ds = SST2ClientDataset(DATA_DIR / f"client_{cid}.csv", tok, max_len=max_len)
...
def make_dev_loader(batch_size: int = 64, max_len: int = MAX_LEN) -> DataLoader:
    tok = _ensure_tokenizer()
    ds = SST2ClientDataset(DATA_DIR / "dev.csv", tok, max_len=max_len)
    return DataLoader(ds, batch_size=batch_size, shuffle=False, drop_last=False)


@torch.no_grad()
def evaluate_sst2(model: nn.Module, dev_loader: DataLoader, device=DEVICE) -> float:
    model.eval()
    correct = 0
    total = 0
    last_logits = None
    ...
    for k in sd_old.keys():
        ...
        if "lora_" in k:
            if diff <= atol:
                ...
                stats["lora_diff"] += 1
                max_diff["lora"] = max(max_diff["lora"], diff)
        else:
            if diff <= atol:
                stats["backbone_same"] += 1
            else:
                stats["backbone_diff"] += 1
                max_diff["backbone"] = max(max_diff["backbone"], diff)

    print(
        "[CHECK] Model copy verification\n"
        f"  LoRA     : same={stats['lora_same']} diff={stats['lora_diff']} "
        f"(max_diff={max_diff['lora']:.3e})\n"
        f"  Backbone : same={stats['backbone_same']} diff={stats['backbone_diff']} "
        f"(max_diff={max_diff['backbone']:.3e})"
    )
...
def attach_dp_to_clients(
    ...
    for cid, client in enumerate(clients):
        ...
        params_A, params_B, params_score_only = select_param_groups_by_layer_modes(
...
def run_federated_training(args):
    ...
    for r in range(1, ROUNDS + 1):
        ...
        acc = evaluate_sst2(server.global_model, dev_loader)
        acc_log.append(acc)
    ...
    mode_df.to_csv(mode_csv_path, index=False)

    pd.DataFrame(
        {
            "round": list(range(1, len(acc_log) + 1)),
            "dev_acc": acc_log,
            "best_acc": np.maximum.accumulate(acc_log),
        }
