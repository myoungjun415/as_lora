...
NUM_CLIENTS = 6
ACTIVE_CLIENTS = 6
DATA_DIR = Path("qnli_federated_6clients_alpha0.5")
...
class QNLIClientDataset(Dataset):
    def __init__(self, csv_path: Path, tokenizer, max_len: int = 128):
        self.df = pd.read_csv(csv_path)
        self.df = self.df.dropna(subset=["question", "sentence", "label"]).reset_index(drop=True)
...
def make_client_loaders(batch_size: int, max_len: int = MAX_LEN) -> List[DataLoader]:
    ...
    for cid in range(NUM_CLIENTS):
        ds = QNLIClientDataset(DATA_DIR / f"client_{cid}.csv", tok, max_len=max_len)
...
def make_dev_loader(batch_size: int = 64, max_len: int = MAX_LEN) -> DataLoader:
    tok = _ensure_tokenizer()
    ds = QNLIClientDataset(DATA_DIR / "dev.csv", tok, max_len=max_len)
    return DataLoader(ds, batch_size=batch_size, shuffle=False, drop_last=False)


@torch.no_grad()
def evaluate_qnli(model: nn.Module, dev_loader: DataLoader, device=DEVICE) -> float:
    model.eval()
    correct = 0
    total = 0
    last_logits = None
...
def log_lora_weight_change_by_mode(client, before, mode: str):
...
def run_federated_training(args):
    ...
    for r in range(1, ROUNDS + 1):
        ...
        acc = evaluate_qnli(server.global_model, dev_loader)
