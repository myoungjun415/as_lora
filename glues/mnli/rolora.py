...
class FederatedServer:
    ...
    def aggregate_mean_filtered(self, client_states: List[Dict[str, torch.Tensor]], mode: str):
        if not client_states:
            return None
...
def make_clients(k: int, server: FederatedServer, lr_a: float, lr_b: float) -> List[FederatedClient]:
    ...
    for i in range(k):
        ...
        opt_A = SGD(params_A, lr=lr_a, weight_decay=0.0)
        opt_B = SGD(params_B, lr=lr_b, weight_decay=0.0)
...
class MNLIClientDataset(Dataset):
    def __init__(self, csv_path: Path, tokenizer, max_len: int = 128):
        ...
        self.df = self.df.dropna(subset=["premise", "hypothesis", "label"]).reset_index(drop=True)
...
def make_client_loaders(batch_size: int = BATCH_SIZE, max_len: int = MAX_LEN) -> List[DataLoader]:
...
def make_dev_loaders(batch_size: int = 64, max_len: int = MAX_LEN):
    tok = _ensure_tokenizer()
    dev_m = MNLIClientDataset(DATA_DIR / "dev_m.csv", tok, max_len=max_len)
    dev_mm = MNLIClientDataset(DATA_DIR / "dev_mm.csv", tok, max_len=max_len)

    return (
        DataLoader(dev_m, batch_size=batch_size, shuffle=False, drop_last=False),
        DataLoader(dev_mm, batch_size=batch_size, shuffle=False, drop_last=False),
    )
...
def clear_grad_samples(model: nn.Module):
    for p in model.parameters():
        if hasattr(p, "grad_sample"):
            p.grad_sample = None


def log_lora_weight_change_by_mode(client: FederatedClient, before: Dict[str, torch.Tensor], mode: str):
...
def mask_lora_grads(model: nn.Module, mode: str):
...
def train_local_steps(cid: int, client: FederatedClient, dataloader: DataLoader, steps: int, mode: str):
    client.model.train()
...
def partial_load_lora_state_dict_(model: nn.Module, server_sd: Dict[str, torch.Tensor], mode: str):
    with torch.no_grad():
        own = model.state_dict()
        ...
        for k, v in server_sd.items():
            ...
            if k_norm in own_keys:
                dst = own[own_keys[k_norm]]
                dst.copy_(v.to(dtype=dst.dtype, device=dst.device))
...
def run_federated_training(args):
    ...
    dev_m_loader, dev_mm_loader = make_dev_loaders()
    ...
    for r in range(1, ROUNDS + 1):
        ...
        previous_mode = AB_SCHEDULE[(r - 2) % len(AB_SCHEDULE)] if r > 1 else mode

        server_lora = server.broadcast()
        for cid in active_ids:
            partial_load_lora_state_dict_(clients[cid].model, server_lora, previous_mode)
        ...
        acc_m = evaluate_mnli(server.global_model, dev_m_loader)
        acc_mm = evaluate_mnli(server.global_model, dev_mm_loader)
        acc_avg = 0.5 * (acc_m + acc_mm)

        best_acc_m = max(best_acc_m, acc_m)
        best_acc_mm = max(best_acc_mm, acc_mm)
        best_acc_avg = max(best_acc_avg, acc_avg)

        print(
            f"[Round {r:03d}] active={active_ids} "
            f"dev_m={acc_m:.4f} dev_mm={acc_mm:.4f} avg={acc_avg:.4f} "
    ...
    print(
        f"\nFinal best results\n"
        f"dev_m={best_acc_m:.4f}\n"
        f"dev_mm={best_acc_mm:.4f}\n"
        f"avg={best_acc_avg:.4f}\n"
    )
