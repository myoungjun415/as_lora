...
ROUNDS = 100
LOCAL_STEPS = 10
DATA_DIR = Path("cifar100_federated_3clients_alpha0.5")
...
def make_client_loaders(batch_size: int) -> List[DataLoader]:
    ...
    for cid in range(NUM_CLIENTS):
        ds = CIFARNPZDatasetProcessor(DATA_DIR / f"client_{cid}.npz", processor)
...
def make_dev_loader(batch_size: int = 64) -> DataLoader:
    processor = _ensure_processor()
    ds = CIFARNPZDatasetProcessor(DATA_DIR / "test.npz", processor)
