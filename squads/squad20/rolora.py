...
DEFAULT_DATA_DIR = Path("squad2_0_federated_6clients_equal")
...
class SQuADv2ClientDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        is_impossible = False
        if "is_impossible" in row.index:
            val = row["is_impossible"]
            if isinstance(val, str):
                is_impossible = val.lower() == "true"
            else:
                is_impossible = bool(val)

        if is_impossible or len(answers.get("text", [])) == 0 or len(answers.get("answer_start", [])) == 0:
            ans_text = ""
            ans_start = 0
            ans_end = 0
        else:
            ans_text = answers["text"][0]
            ans_start = int(answers["answer_start"][0])
...
def evaluate_squad_v2_em_f1(
    ...
    for ex_id, feat_indices in features_per_example.items():
        ...
        for fi in feat_indices:
            ...
            cls_null_score = float(start_log[0] + end_log[0])
            if cls_null_score < min_null_score:
                min_null_score = cls_null_score
    ...
    references = [
        ...
        for ex in squad_dev_hf_dataset
    ]
...
def run_federated_training(args):
    ...
    data_dir = Path(args.data_dir) if hasattr(args, "data_dir") and args.data_dir else DEFAULT_DATA_DIR
    ...
    client_loaders = make_client_loaders(data_dir=data_dir, batch_size=args.batch_size, max_len=MAX_LEN)
