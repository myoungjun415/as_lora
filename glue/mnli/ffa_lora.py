...
def train_local_steps(client: FederatedClient, dataloader: DataLoader, steps: int = LOCAL_STEPS):
    client.model.train()
...
def run_federated_training(args):
    ...
    clients = make_clients(NUM_CLIENTS, server, lr=args.lr_b)
    ...
    for r in range(1, ROUNDS + 1):
        ...
        log_rows.append({
            "round": r,
            "dev_m_acc": acc_m,
            "best_m_acc": best_acc_m,
            "dev_mm_acc": acc_mm,
            "best_mm_acc": best_acc_mm,
        })

        print(
            f"[Round {r:03d}] active={active_ids} "
            f"dev_m_acc={acc_m:.4f} dev_mm_acc={acc_mm:.4f} "
            f"agg_keys={len(agg_state) if agg_state else 0}"
        )

    print(f"\nFinal best dev_m_acc = {best_acc_m:.4f}")
    print(f"Final best dev_mm_acc = {best_acc_mm:.4f}\n")
